...
description: 프로덕트 매니저(PM)가 바이브코딩으로 UI/UX를 설계하거나, 사용자 여정을 구성하거나, 프로덕트의 화면·흐름·인터랙션을 만들 때 제품 심리학 원칙을 자동으로 적용하는 스킬. 6P 스토리보드, BMAP 행동모델, B.I.A.S 프레임워크, Peak-End 저니맵, 윤리적 디자인 체크, 스크린샷 기반 UI 진단을 실무에 바로 적용한다. 다음 상황에서 반드시 이 스킬을 사용하라 — 사용자 여정 설계, 온보딩 플로우 설계, 전환율 개선, CTA 배치, 랜딩페이지 구성, 회원가입/결제 플로우, 푸시알림/이메일 문구 작성, 가격 페이지 설계, 기능 우선순위 결정, UX 리뷰, 스크린샷 UI 진단, 사용자 이탈 분석, A/B 테스트 가설 수립, 프로토타입 제작, 와이어프레임 리뷰, 제품 기획서 작성 등 프로덕트 설계 및 개선에 관련된 모든 작업. '사용자 심리', '행동 설계', '전환 최적화', '그로스', 'UX 개선', '화면 진단', '포스트잇식 리뷰', '점수화' 등의 키워드가 나와도 이 스킬을 사용하라.
---
...
- **스크린샷 진단**: 화면을 점수화하고 포스트잇식 문제 라벨, 우선순위, 개선 방향을 제시
...
1. 보이는 화면에서 확인 가능한 문제만 말한다.
2. Copy / Focus / Hierarchy / Friction / Reward 5축으로 1-5점 점수화하되, 각 축을 B.I.A.S/BMAP와 연결해 해석한다.
...
7. Product Psychology Lens 섹션에서 적용한 이론(B.I.A.S, BMAP, Peak-End, cognitive load, information scent 등)을 명시하고, 각 이론이 화면에서 어떻게 드러나는지 설명한다.
...
| `references/screen-diagnostic.md` | 스크린샷 기반 UI 진단 루브릭, 점수표, sticky-note 출력 템플릿 | 화면 진단, 포스트잇식 critique, 점수화 요청 시 |
